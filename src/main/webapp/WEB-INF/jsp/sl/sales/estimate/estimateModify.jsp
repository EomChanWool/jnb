...
<body id="page-top">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800">견적서 수정</h1>
                    <!-- DataTales Example -->
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <div class="table-responsive">
                            	<form action="${pageContext.request.contextPath}/sl/sales/estimate/modifyEstimateOk.do" name="modifyForm" method="post">
                            		<input type="hidden" name="esIdx" value="${estimateVO.esIdx}">
	                                <table class="table table-bordered" id="dataTable">
	                                    <tbody>
										<tr>
											<th>견적서 제목  <span class="req">*</span></th>
											<td><input type="text" class="form-control" name="esName" value="${estimateVO.esName}"/></td>
											...
											<td>
												<select class="form-control" name="cIdx" id="cIdx">
													...
													<c:forEach var="list" items="${compnayList}" varStatus="status">
														<option value="${list.cIdx}" <c:if test="${estimateVO.cIdx eq list.cIdx}">selected="selected"</c:if>>${list.cName}</option>
													</c:forEach>
												</select>
											</td>
											...
											<td>
												<select class="form-control" name="aIdx" id="aIdx">
													...
													<c:forEach var="list" items="${accountList}" varStatus="status">
														<option value="${list.acCode}" <c:if test="${estimateVO.aIdx eq list.acCode}">selected="selected"</c:if>>${list.acName}</option>
													</c:forEach>
												</select>
											</td>
										</tr>
										...
										<tr>
											<th>비고</th>
											<td colspan="3"><textArea name="esNote">${estimateVO.esNote}</textArea></td>
										</tr>
									</tbody>
	                                </table>
	                                <table class="table table-bordered" id="dataTable">
										...
										<tbody class="prodList" style="display: none;">
											...
											<c:forEach var="list" items="${prList}" varStatus="status">
												<tr>
													...
													<td>
														<input type="text" class="form-control prod" name="prod${status.count}" id="prod${status.count}" list="productList" <c:if test='${list.prod != "null"}'>value="${list.prod}"</c:if> autocomplete="off" style="text-align: center;">
														<datalist id="productList">
															<c:forEach var="list2" items="${prodList}" varStatus="stat">
																<option value="${list2.itemCd}">${list2.itemName}</option>
															</c:forEach>
														</datalist>
													</td>
													<td><input type="text" class="form-control cnt" name="cnt${status.count}" id="cnt${status.count}" <c:if test='${list.cnt != "null"}'>value="${list.cnt}"</c:if> style="text-align: center;"></td>
													<td><input type="text" class="form-control perPrice" name="perPrice${status.count}" id="perPrice${status.count}" <c:if test='${list.perPrice != "null"}'>value="${list.perPrice}"</c:if> style="text-align: right;"></td>
												</tr>
											</c:forEach>
											<tr>
                                ...
                                <div class="btn_bottom_wrap">
									<button type="submit" class="btn_ok" onclick="fn_modify_estimate()" style="border:none;">확인</button>
	...
	<script>
	function fn_modify_estimate(){
		const num = /^\d+$/;
		
		if(modifyForm.esName.value == ''){
			alert("견적서 제목을 확인 바랍니다.");
			return;
		}
		
		if(modifyForm.esComManager.value == ''){
			alert("자사 담당자를 확인 바랍니다.");
			return;
		}
		
		if(modifyForm.cIdx.value == ''){
			alert("사업장을 확인 바랍니다.");
			return;
		}
		
		if(modifyForm.aIdx.value == ''){
			alert("거래처를 확인 바랍니다.");
			return;
		}
		
		if(modifyForm.prod1.value == '' || !num.test(modifyForm.cnt1.value) || !num.test(modifyForm.perPrice1.value)){
	...
	function calcTotalPrice(){
		...
		modifyForm.totalPrice.value = total;
